 ...
 * Currencies Model
 */
class LocationdataModelCurrencies extends JModelList
{
	...
	{ 
		...
		if (LocationdataHelper::checkArray($items))
		{
			...
			{
				$access = ($user->authorise('currency.access', 'com_locationdata.currency.' . (int) $item->id) && $user->authorise('currency.access', 'com_locationdata'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__locationdata_currency");
	...
	*/
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__locationdata_currency'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__locationdata_currency'))->set($fields)->where($conditions); 

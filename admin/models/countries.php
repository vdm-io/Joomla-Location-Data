 ...
 * Countries Model
 */
class LocationdataModelCountries extends JModelList
{
	...
	{ 
		...
		if (LocationdataHelper::checkArray($items))
		{
			...
			{
				$access = ($user->authorise('country.access', 'com_locationdata.country.' . (int) $item->id) && $user->authorise('country.access', 'com_locationdata'));
				if (!$access)
				{
					unset($items[$nr]);
					continue;
	...
	protected function getListQuery()
	{
		...
		$query->join('LEFT', $db->quoteName('#__locationdata_currency', 'g') . ' ON (' . $db->quoteName('a.currency') . ' = ' . $db->quoteName('g.codethree') . ')');
	...
	*/
	public function getExportData($pks)
	{
		...
		if (LocationdataHelper::checkArray($pks))
		{
			...
			// From the locationdata_country table
			$query->from($db->quoteName('#__locationdata_country', 'a'));
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = JFactory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__locationdata_country");
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				// Check table
				$query->update($db->quoteName('#__locationdata_country'))->set($fields)->where($conditions); 

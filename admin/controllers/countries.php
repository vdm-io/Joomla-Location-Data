 ...
 */
class LocationdataControllerCountries extends JControllerAdmin
{
	protected $text_prefix = 'COM_LOCATIONDATA_COUNTRIES';
	/**
	 * Proxy for getModel.
	 * @since	2.5
	 */
	public function getModel($name = 'Country', $prefix = 'LocationdataModel', $config = array())
	{
	...
	public function exportData()
	{
		...
		if ($user->authorise('country.export', 'com_locationdata') && $user->authorise('core.export', 'com_locationdata'))
		{
			// Get the input
			...
			if (LocationdataHelper::checkArray($data))
			{
				...
				LocationdataHelper::xls($data,'Countries_'.$date->format('jS_F_Y'),'Countries exported ('.$date->format('jS F, Y').')','countries');
			}
		}
	...
	public function importData()
	{
		...
		$user = JFactory::getUser();
		if ($user->authorise('country.import', 'com_locationdata') && $user->authorise('core.import', 'com_locationdata'))
		{
			...
			if (LocationdataHelper::checkObject($headers))
			{
				...
				$message = JText::_('COM_LOCATIONDATA_IMPORT_SELECT_FILE_FOR_COUNTRIES');

...
/**
 * Locationdata Route Helper
 **/
abstract class LocationdataHelperRoute
{
	protected static $lookup;

	/**
	* @param int The route of the Exchangerates
	*/
	public static function getExchangeratesRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'exchangerates'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_locationdata&view=exchangerates&id='. $id;
		}
		else
		{
			// Initialize the needel array.
			...
			$link = 'index.php?option=com_locationdata&view=exchangerates';
		}
		if ($catid > 1)
		{
			$categories = JCategories::getInstance('locationdata.exchangerates');
	...
	/**
	 * Get the URL route for locationdata category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_locationdata&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_locationdata');
		...
		if ($active
			&& $active->component == 'com_locationdata'
